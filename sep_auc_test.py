...
判读
{'='*78}
  · sep-AUC ≈ 0.5           → 表示根本分不出处境 ⇒ 实验③ 的四臂无差异有了解释,
                               而且必须先修表示(换池化/换层/换度量), ③ 才有意义;
  · 同处境与不同处境的相似度【差距】很小(<0.05) → 各向异性: 向量全挤在窄锥里;
  · 若 last-token 明显优于 mean → 直接把 ③ 和方案里的池化换掉;
  · 有效邻居数 ≈ 1 或 ≈ 全体   → τ 选得不对, 软分组退化成"只听一个"或"全平均".""")
