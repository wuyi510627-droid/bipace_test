...
for sd in SEEDS:
    ...
    E_raw = embed(raw_obs)
    bipace_raw = ca_acc(soft_qv(E_raw, act, ret, TEMP), isp1, act)
    ...
    for C in Cs:
        ...
        row["BiPACE_comp"].append(ca_acc(soft_qv(E_comp, act, ret, TEMP), isp1, act))
...
print(f"\n===== 均值±标准差 (over {len(SEEDS)} seeds) =====")
for i, C in enumerate(Cs):
    tag = f"C={C:.2f}" if C is not None else "LLM摘要"
    print(f"{tag} | " + "  ".join(f"{m}={mean[m][i]:.3f}±{std[m][i]:.3f}" for m in METHODS))
...
if MODE=="cheap":
    ...
    for m,(fmt,lab) in sty.items():
        plt.errorbar(C_LEVELS, mean[m], yerr=std[m], fmt=fmt, capsize=3, label=lab)
...
cc_err = std["CompCeil"][-1]; bc_err = std["BiPACE_comp"][-1]
print("\n===== 裁决 (看最压处) =====")
