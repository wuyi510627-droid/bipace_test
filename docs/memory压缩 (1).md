...
> **验证状态**：⬜ **一条都没验过**。代码已写（`credit.py`，自测通过），实验⑤脚本已就绪（`ca_ablation_test.py`），但**还没在数据上跑**。读时请把本节当作待检验的方案，不是结论。
...
| **②** | 信号可信度 | 探针问出来的"能不能成"靠不靠谱 | 🔧 脚本已写待跑 | 合成可跑；真实更好 |
| **⑤** | CA 侧消融 | §3 那四条改法各自值不值 | 🔧 脚本已写待跑 | 合成可跑；真实更好 |
...
### ② 信号可信度检验 —— 验地基 🔧【脚本已写：`signal_check_test.py`】
...
### ⑤ CA 侧消融 —— 证明 §3 那四条不是白加的 🔧【脚本已写：`ca_ablation_test.py`】
...
| ⑤ | **CA 侧消融**（软分组 / 保真度加权 / PACE 逐条开关） | 🔧 脚本已写待跑 |
