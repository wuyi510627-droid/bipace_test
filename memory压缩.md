...
**对照臂**（同一个任务，喂给 agent 的记忆不同）

| 代号 | 是什么 | 代表谁 |
|---|---|---|
| **vtree** | **本方法**：按价值跳幅分配 token 的多分辨率压缩 | ⭐ 我们 |
| `vtree_val` / `vtree_bel` | vtree 的两种信号：**价值**（能不能成）/ **信念**（隐藏事实是什么） | 主臂 / 对照臂 |
| **full** | 全历史，一字不压 | **上界**（准但贵） |
| **no_mem** | 只给当前这一步观测，不给历史 | **下界**（省但没用） |
| **trunc-$k$** | 只保留**最近 $k$ 步**，更早的直接截掉 | GraphGPO / G2PO 处理长序列的做法 |
| **naive** | 整段压成一句泛泛的话（如"agent 在厨房操作了一番"） | 笨压缩的下界 |
| **random-$k$** | 同样的 token 上限，但**随机**挑保留哪几步 | "不会挑"的基线 —— 用来证明会挑有价值 |
...
| **信用集中度** | $A_{\text{关键步}}\big/\sum_t \lvert A_t\rvert$ | 关键步拿走了多大比例的功劳，越高越好 |
| **margin** | $A_{\text{关键步}}-\max_{t\neq\text{关键}}A_t$ | 关键步领先第二名多少，衡量分得干不干脆 |
...
**问题**：$c_{\text{摘要}}=0.6$、$c_{\text{折叠}}=0.3$ 是拍脑袋填的（§3.2②）。它们直接决定保真度，而保真度又进信用分配——**一个没根据的常数在影响训练信号**。

**三种定法，选第三种**：

| 定法 | 怎么算 | 问题 |
|---|---|---|
| ① 按长度比 | $c_t=\dfrac{\text{压缩后 token}}{\text{原文 token}}$（摘要实测 $9/21\approx0.43$） | 只反映留了多少**字**，不反映留的是不是**要紧**的字 —— 摘要专保主干，长度比会**系统性低估**它 |
| ② 每步实测 | $c_t=1-\lvert V(\text{原文})-V(\text{压缩版})\rvert$ | 最准，但每步多问一次探针，在线不可行 |
| ⭐ **③ 一次标定** | 用 ② 的公式跑一批数据取均值，**得到常数后固定用** | **② 的准确性 + ① 的零在线成本** |

**做法**：取数百条真实观测，每条分别以**原文 / 摘要 / 折叠**三种形式喂探针，统计价值判断的一致性：

$$c_{\text{摘要}} = \operatorname*{mean}_t\Big(1-\big|V(\text{原文}_t)-V(\text{摘要}_t)\big|\Big)$$

**很便宜**：不训练、只跑探针，可与实验②合并做。

**它还顺带回答一个我们不知道的问题**：摘要到底保住了多少判断力？

- 若标定出 $c_{\text{摘要}}\approx 0.9$ → 摘要几乎不损失判断力，**中间档很划算**；
