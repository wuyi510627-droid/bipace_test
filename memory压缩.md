...
保真度低的情形：邻居太少 / 组内回报方差大 / **该步在树上被压到很粗的层（分辨率低 ⇒ memory 不可靠）**。
...
| **4** | 两档还是多档 | **三档** | 三条技术理由（§2.4，含实测）：① 两档是 0-1 背包，token 零头用不掉——实测同 token 上限下三档多保住一条线索；② 跳幅是连续量，二值化把中间地带的步逼着二选一；③ ****保真度** 需要"在但不可靠"这个状态，否则 §3.2② 的保真度加权不成立**。边界：token 充足或极紧时三档 $=$ 两档，优势只在中间区间 |
