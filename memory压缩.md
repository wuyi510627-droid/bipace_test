...
| **③** | 信用精度 | **压完之后信用还分得对** ← 核心证据 | 🔧 脚本已写<br>`credit_rank_test.py` | — |
...
> ⚠️ **④ 必须排在 ③ 前面**。③ 首轮跑出来四个臂几乎无差异（信用集中度 0.28，贴着"全糊掉"那一档），
...
### ③ `credit_rank_test.py` —— 核心证据 🔧【脚本已写，待跑】
...
### ④ 可分性检验 —— 压完还认得出同处境吗 🔧【脚本已写：`sep_auc_test.py`，待跑】
...
**它同时是 ③ 的诊断工具**：③ 首轮四臂无差异，就是靠这个实验定位根因。脚本额外做三项诊断 ——
① **相似度分布**（是否全挤在 0.9+ 的窄锥里，即各向异性）；
② **池化方式对比**（mean vs last-token —— BiPACE 用的是后者）；
③ **核宽 $\tau$ 敏感性**（有效邻居数随 $\tau$ 怎么变，现用的 0.05 合不合适）。
