...
**推论（决定了 §4 决定 5）**：既然是 token 硬上限，压缩就必须是**固定 token 上限**，不能是固定比例——固定比例下 token 仍随轨迹长度线性增长，照样超限。
...
**这一步白送的东西**：token 上限固定 ⇒ 各档必须**互相竞争** ⇒ 压缩率可连续调节 ⇒ **这就是 IB 里 $\beta$ 旋钮的物理意义**（见 §5）。固定比例给不了这个，因为每条轨迹各留各的，互不相干。
...
| | 公式 | 解决什么 | 定位 |
|---|---|---|---|
| **A. 分位数** | $\tau=\operatorname{quantile}_p(\{d_{ij}\}),\ p\approx0.2$ | **尺度漂移**：量的始终是"在当前这批里算不算近"，相对而非绝对 | ⭐ **默认** |
| **B. k-NN** | $\tau_i=d(\varphi_i,\varphi_{(k)})$ | 密度不均：稠密处自动收紧、稀疏处放宽（t-SNE 的 perplexity 思路） | 暂不做 |
| **C. 保真度** | $\tau_i=\tau_0/\text{保真度}_i$ | **压得狠 ⇒ 表示不可靠 ⇒ 距离测量本身有噪声 ⇒ 放宽标准** | ⭐ 消融项 |

**C 只有做压缩的方法能做**，而且它和 ②的"降权"是**两种不同的机制**：

| | 做什么 | 含义 |
|---|---|---|
| ② 降权 | $w'_j=w_j\times\text{保真度}_j$ | 这个邻居**少听一点** |
| **C 调核宽** | $\tau_i\propto 1/\text{保真度}_i$ | **"多远算近"的标准变了** |

方向甚至相反：压得狠的点，降权是"少信它"，调核宽是"**放宽**对它的距离要求"（因为它的距离测不准）。所以 C 值得单独验——它可能比降权更站得住（`walkthrough.py` 已观察到降权在某些情形下是冗余的，见 §9 风险⑥）。
...
$$\boxed{\text{保真度}=\frac{\sum_t w_t\, c_t}{\sum_t w_t}}\qquad w_t=\text{第 }t\text{ 步原文的 token 数}$$
...
$$\text{保真度}=\frac{10\times0.3+14\times1.0+13\times1.0}{10+14+13}=\frac{30}{37}=0.81$$
...
$$\text{保真度}=\frac{\sum_t \Delta_t\, c_t}{\sum_t \Delta_t}
...
| **4** | 两档还是多档 | **三档** | 三条技术理由（§2.4，含实测）：① 两档是 0-1 背包，token 零头用不掉——实测同 token 上限下三档多保住一条线索；② 跳幅是连续量，二值化把中间地带的步逼着二选一；③ **保真度需要"在但不可靠"这个状态，否则 §3.2② 的保真度加权不成立**。边界：token 充足或极紧时三档 $=$ 两档，优势只在中间区间 |
| **5** | 固定比例还是固定token 上限 | **固定 token 上限** | 固定比例下 token 仍随 $L$ 线性增长 → 照样超限 → **动机自毁**；且实验图上 vtree 会变成斜线，说服力大减 |
...
| c. 软分组 + **保真度** | ✓ | **✓** | ✗ | **★保真度加权的净增量** |
| d. 全套 | ✓ | ✓ | ✓ | PACE 再加多少 |
| e. 全套 + **自适应核宽** | ✓ | ✓ | ✓ | **固定 $\tau$ vs 分位数自适应 vs 保真度自适应**（§3.2① 的 A / C）|
