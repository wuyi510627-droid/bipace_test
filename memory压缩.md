...
可分性 AUC（cos 能否预测"是否同一处境"；1=干净可分、0.5=瞎猜），5 seed、SEM≈0.00：

| 臂 | 原始不压 | 忠实压缩(C=1) |
|---|---|---|
| **BiPACE 忠实(last-token)** | 0.67 | **0.93** |
| 对照(mean 池化) | 0.52 | 0.92 |
| **A2 粗糙压缩(真 7B 摘要)** | —— | **≈0.56（白压）** |

1. **压缩 > 不压缩，稳、且两种表示都成立**：原始 0.52–0.67 → 忠实压缩 ~0.93。→ 正面回答"证明压缩比不压好"，而且用 **BiPACE 忠实的 last-token** 也成立（不是池化假象）。
2. **但有前提——只有忠实压缩赢**：真 7B 随手摘要只有 **0.56 ≈ 不压**。**0.56→0.93 的鸿沟 = 方法要吃的空间**。
3. **真摘要器会失败**：7B 零样本摘要经常把干扰当主体、丢任务核心（如"填邮箱"被摘成"展示促销推荐"），同处境不同变体还互相矛盾。

⚠️ **一处要讲清的细节（= (b) 的动机）**：压缩后短文本的隐状态被挤进一个很窄的**锥形**里——**排序信息还在**（AUC 高达 0.93，last-token/mean 都不坍），但 **BiPACE 的固定阈值贪心聚类会一刀切、把不同处境全并成一簇**（cluster_probe 证实）。即：**信息没丢，是 BiPACE 的阈值度量抓不住**。→ 该做的是**对锥形坍缩鲁棒的度量/自适应阈值**（偏 (b)），而非救回丢失的信息（(a)）。绝对增益仍由 M1（真 memory）最终确认。
