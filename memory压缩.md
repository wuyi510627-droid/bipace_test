# Memory 压缩方案定版 —— B：价值失真记忆树

> **状态**：方向已由导师拍板（"对压缩后 memory 做信用分配可以做，但**创新点必须落在压缩方法本身**"）。三候选 A/B/C 中选定 **B**，七条设计决定已定版（见 §3）。
> **历史**：本文件原为"memory 表示空间软分组信用分配"方案（软分组 + 保真度加权）。该方向的核心（软分组）被 BiPACE 2026-06 占位，且导师要求创新落在压缩上，故主体已替换为 B。旧方向仍然有效的调研事实与工程定位保留在 §9。

---

## 0. 一句话

> **给定固定的 token 预算，按"这一步让'能不能成'跳了多少"，决定轨迹每一段展开到多细。**

区别于所有现有压缩方法的一句话：**它们为"压完还能检索/答题"而压，B 为"压完还能把信用分对"而压。压缩的目标函数换了。**
...
→ **"记忆长 ⇒ 需要压缩"这条论证是假的。** 如果 B 的动机写成"长程记忆很难，所以要压缩"，一句话就被 Ni 2023 打穿。
...
已扫的 ~24 篇 memory 压缩论文（ICML 2026 folder 30 等），目标清一色是"压完还能**检索** / **答题** / **拿任务奖励**"。

**没有一篇是为了"压完还能把信用分对"去压的。** 压缩的**目标函数**这一格是空的 —— 这就是 B 的立足点。
...
flowchart TD
    ...
    C --> D["★创新★ ③ 固定 token 预算下<br/>按 Δ 贪心分配分辨率<br/>(原文 / 一句摘要 / 整段合并)"]
    D --> E["④ 压缩后 memory<br/>→ 编码 → 凑堆"]
    E --> F["⑤ 堆内比动作结局 → 每步优势"]
    F --> G["⑥ 逐步信用喂 GRPO 训练"]
```

**新旧分工**：④⑤⑥ 是**借来的容器**（GiGPO / BiPACE / HGPO，导师已批准）；①②③ 是**唯一要创新的一块**，且是让 ④ 凑得成堆的前提。
...
**这个量的血统**：**PER**（Prioritized Experience Replay）已经在按 TD-error（= 价值惊讶）分配**回放优先级**；B 把同一原则从"回放优先级"升级成"**压缩分辨率**"。同一句话"把资源砸在意外/重要处"，换了作用对象。
...
| # | 问题 | 定案 | 排除了什么 / 为什么 |
|---|---|---|---|
| **1** | 尺子用**价值**还是**信念** | **价值**（能不能成） | 信念（对隐变量的后验）降格为"价值为什么变"的解释。理由：① B 目标是保住**信用**，信用=优势=价值差，同量纲才接得上理论；② "按信念变化压"已被 **∆Belief-RL** 占位；③ 信念变但价值不变的信息（"朋友养了猫"）不该保留 |
| **2** | 价值从哪来 | **问大模型**（主线）<br>组内分歧 → 讨论 + 可选 ablation | 排除 critic（与 critic-free 容器不兼容）；排除 MC 回报（稀疏奖励下退化成"离终点多远"→ **撞 GraphGPO**） |
| **3** | 跳幅 vs 压缩前后差 | **跳幅 = 压缩规则**<br>**压缩前后差 = 评估指标** | "压掉后判断会不会变"正是 **C 方案（因果充分压缩）的定义**，当规则会让 B 塌成 C；且每试一种压法要重问一遍模型，贵得离谱。但它是 IB 里"失真"的正统定义，留作评价体系 |
...
**立足点要讲清**：B 不是发明 IB（1999 年的成熟框架），B 的三点是：
1. 把 IB 的 $Y$ **换成信用/价值** —— CA 主线五篇都没用过 IB；
2. 用 **PER 的价值跳幅**把 IB 抽象的 $D_{\text{KL}}$ 失真落地成可逐步算的量；
3. **理论亲戚**：Abel 的保价值抽象证明"合并价值相近的状态，价值损失有界" → 可改写成"**压缩到什么程度，信用误差有界**"，这是 B 潜在的理论卖点。
...
| 论文 | 它做什么 | **和 B 的分界线** |
|---|---|---|
| **BiPACE** | 隐状态 cosine 软聚类分组 + 动作反事实 | ① 它是**横向**合并状态空间，B 是**纵向**变时间轴粒度；② 它挂 bisimulation 的名、干 **朴素 cosine** 的事（不查奖励/转移/递归/无理论保证），B 用价值跳幅比它更接近"保价值合并"；③ **它自己把 B 写成 future work** |
| **HGPO** | 层级分组（历史步数分层） | 承认 memory 压缩后层级分组 intractable；B 的树的粗/细层正好可以当它的层级 |
| **GraphGPO** | 状态转移图 + 反向 Dijkstra 距离 | **最危险的碰撞**：若 B 用 MC 回报当价值，跳幅退化成"离终点多远"= GraphGPO。**这是 §3 决定 2 排除 MC 的直接原因** |
| **G2PO / GEPO** | 图上做信用分配（边 TD / 介数中心性×语义） | 都在**单局内做信用分配**，不碰压缩；且 G2PO 用 truncate 处理长序列 → B 的 T-maze 实验正是打它这个地基 |
| **∆Belief-RL** | 按**信念变化**做信号 | **§3 决定 1 就是为躲它**：B 锚价值不锚信念，且把信念收编为"价值变化的原因" |
| **MemoPilot** | 训记忆模型写 selective memory，多轮 GRPO | ① **轮级**（一局一份笔记）vs B **步级**；② 自认 Limitation 2：长程压缩靠预处理、非端到端 → **B 就是它这条 limitation 的答案**；③ 它的"Full History 反而更差"是 B 动机的现成实验证据 |
| **Mem-T** | MoT 树把终局分稠密化 + hindsight 回传到记忆操作 | 信号是 **QA 的 F1 / 证据密度（文本重合）** → 压缩改变文本后信号就失灵，所以它把压缩推给预处理。B 用**不依赖文本表面**的价值跳幅，压缩才可学 |
| **HIPIF** | 分层规划 + 子目标完成后折叠历史 | **事件驱动**（子目标做完就折叠）vs B **信用驱动**；**子目标级整块折叠** vs B **步级连续粒度**。子目标内部关键步和平坦步混着，一刀切会连关键步一起折 |
| **RGMem / Memora** | 多尺度粗粒化 / 抽象-具体平衡 | 壳一样（多分辨率），**目标不同**：它们为检索/个性化，B 为信用 |
| **PER** | 按 TD-error 分配回放优先级 | **B 的种子**，不是竞品：同一原则从"复习谁"换成"压谁" |
| **Ferns / DBC / MICo** | bisimulation 度量（行为等价） | **方法论模板，不是公式套用**：借它们"距离由任务定义、从采样中学、省算力但保上界"这套做法；B 的距离是价值跳幅，不是 $\mid R\mid+\gamma W_1$ |
| **RUDDER / CCA** | 回报分解 / 反事实信用 | RUDDER 的"哪步让预测突变"= 跳幅的另一种算法（LSTM 而非问模型），可当备选量法；CCA 是 **C 方案**的主干 |
...
### ③ `credit_rank_test.py` —— B 的核心证据【待建，最大的洞】
...
| — | Active T-Maze（多候选关键步，B 的真本事图） | ⬜ 钩子已写在 `tmaze_text_test.py` 末尾 |
...
| ③ | **Passive T-maze 太送分** | 关键步唯一且明显，选不错 | 已知问题，脚本注释里写明。**必须做 Active / 多线索版**才算证了 B 的真本事 |
...
→ **合成实验只作机制原型和方向勘探，不作论文证据。** 这条对 B 同样适用。
