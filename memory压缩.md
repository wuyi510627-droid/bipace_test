...
flowchart TD
    subgraph A["阶段 A · rollout 内（每走一步都做一遍）"]
        ...
        J --> B1["③ 固定预算下重排分辨率<br/>跳幅大的优先展开<br/>(原文 / 摘要 / 折叠)"]
        B1 --> M["④ 渲染出 memory_t<br/>顺带产出 conf_t（每步压得多粗）"]
    ...
    G1 --> C1["⑧ conf 加权: w'_j = w_j · conf_j<br/>压得粗的邻居降权"]
...
**注意 ④ 那个"顺带产出"**：`conf` 不是额外算的，是压缩过程本身就知道的（每步落在哪一档）。它一路带到 ⑧ 去给邻居降权——**这就是压缩和信用分配咬合的地方**（§3.3）。
...
$$V_t=\frac{P(\text{是})}{P(\text{是})+P(\text{否})}=\frac{0.28}{0.28+0.12}=0.70$$
...
**但这不影响方法**：我们只用**跳幅**（相对变化），不用绝对值。只要能分出"这步比那步跳得多"，排序对了就够（§4 决定 2）。

**"排序对不对"本身是假设** → 实验②专门验它（见 §7）。
...
**① 预算利用率：两档是 0-1 背包，零头用不掉**

两档下每步要么全占预算、要么完全不占。当剩余预算装不下某步的原文时，这步只能整条丢，剩的零头浪费。中间档能用这点零头保住主干。

**实测**（`vtree_compressor.py` 自测场景 4，三条线索跳幅 0.40 / 0.25 / 0.07）：

| 预算 | 三档保住 | 两档保住 | |
|---|---|---|---|
| 200 / 140 | A B C | A B C | 预算充足，两者相同 |
| **110** | **A B C** | A B | **三档多一条**（C 降级为摘要而非丢弃） |
| 90 | A B | A B | 相同 |
| **70** | **A B** | A | **三档多一条** |
| 50 | A | A | 预算极紧，两者相同 |
...
**③ $\mathrm{conf}$ 需要分辨率 —— 这是 §3.2② 的前提条件**

两档下，被删的步**根本不在 memory 里**，不参与分组，$\mathrm{conf}$ 退化成"在 / 不在"，保真度加权无从谈起。

**只有三档才存在"在、但不可靠"这个状态**（摘要档，$\mathrm{conf}=0.6$）——而 §3.2② 那条"压得粗的邻居降权"正是靠它。**中间档不是装饰，是 CA 侧那条创新的必要条件。**

**代价与边界（诚实说）**：

- 多一个摘要器组件，多一个失败点；
- **摘要改写错比直接删更隐蔽**——删掉了你知道它没了，改错了你以为还在；
- **预算充足或极紧时，三档 $=$ 两档**（见上表）。优势只在**中间区间**。如果实际部署的预算总是很宽裕，中间档确实是多余的。
...
- **当前步不占历史预算**：最后一步永远是原文（agent 要拿它决策）。若让它挤占预算，当前观测一长（网页文本）就会把关键步挤掉，出现"预算越紧越丢关键步"的反向行为。
- **跳幅 $\approx 0$ 的步不升档**：即使预算有富余也不给它。否则贪心会把预算花光在零信息的步上，token 曲线虚高，违背"比特花在影响决策处"。

### 2.6 跟着一条轨迹走一遍（加热番茄，预算 65）

**阶段 A：rollout 内，每走一步做一遍 ①→④**

| $t$ | 观测 | $V_t$ | $\Delta_t$ | 最终档位 | $\mathrm{conf}$ |
|---|---|---|---|---|---|
...
- **第 4 步跳幅（0.07）比第 2 步（0.05）大，却被压得更狠**。原因是第 4 步的原文长（19 字）、第 2 步短（13 字）——同样的预算，短的装得下、长的装不下。**长度参与了竞争**。
...
| 轨迹 | 在该处境的动作 | 终局回报 $R$ | 该步 $\mathrm{conf}$ |
|---|---|---|---|
| #1 | 放进微波炉 | 1（成功） | 1.0（原文） |
| #2 | 放进微波炉 | 1（成功） | 1.0（原文） |
...
- **⑧ conf 加权**：#4 那步被压到折叠档、memory 不可靠 → $w'_4 = w_4\times 0.3$ **降权**，防止它把噪声灌进基准；
- **⑨ 算优势**：
  - $\hat V(s)=\sum_j w'_j R_j$ ≈ 加权平均回报（约 0.6）
  - $\hat Q(s,\text{放进微波炉})$ = 只看同样"放进微波炉"的邻居 ≈ 0.7
  - $A=\hat Q-\hat V\approx +0.1$ → **"放进微波炉"这个动作拿到正信用**；
  - 对 #3 的"走开"：$\hat Q\approx 0$，$A\approx-0.6$ → **负信用**。

**这里就能看出 conf 的作用**：如果不降 #4 的权，它那条"放进微波炉却失败了"的记录会把 $\hat Q$ 拉低，让正确动作的信用变小。而 #4 之所以失败，很可能与该处境无关（后面才出的错）——它的 memory 又恰好被压得最粗、最不可信。**conf 让这条噪声的影响自动变小。**
...
| **4** | 两档还是多档 | **三档** | 三条技术理由（§2.4，含实测）：① 两档是 0-1 背包，预算零头用不掉——实测同预算下三档多保住一条线索；② 跳幅是连续量，二值化把中间地带的步逼着二选一；③ **$\mathrm{conf}$ 需要"在但不可靠"这个状态，否则 §3.2② 的保真度加权不成立**。边界：预算充足或极紧时三档 $=$ 两档，优势只在中间区间 |
...
| | 事项 | 状态 |
|---|---|---|
| ① | tmaze 动机图（token 计数 / 3 seeds / 价值信号 / 去作弊） | ✅ **已跑完**，结果见 §7①；断崖精确落在 $L>k$，$L{=}16$ 省 5.3~7.5× token |
| ② | 信号可信度检验 | ⬜ 待建 |
