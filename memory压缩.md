...
> 正解是**一次性标定**——见 §6 实验⑥。在标定出来之前，凡是依赖这两个数的结论（包括 §2.4 三档的成本收益账）都只能算**定性**成立。
...
| **长度比调整** | $c_t=c^{\ast}_{\text{tier}}\times\dfrac{r_t}{\bar r}$（$c^{\ast}_{\text{tier}}$=该档标定值，$r_t$=本次压缩比，$\bar r$=该档平均压缩比） | **零**（长度已知），但只是粗代理 |
...
我们没有环境模型（拿不到真的 $R$、$P$），所以照 MICo 的路子做采样近似 —— 用探针的价值判断代替，并且用**排序**而非数值来量（免疫模型的乐观/保守偏置）。这条正是 `calibrate_ct.py` 里实现的。
...
| **信用集中度** | $A_{\text{key}}\big/\sum_t \lvert A_t\rvert$ | 关键步拿走了多大比例的功劳，越高越好 |
| **margin** | $A_{\text{key}}-\max_{t\neq \text{key}}A_t$ | 关键步领先第二名多少，衡量分得干不干脆 |
...
| **⑥** | 标定档位分 $c_t$ | 把拍脑袋的 0.6/0.3 换成实测值 | 🔧 脚本已写<br>`calibrate_ct.py` | — |
| **★** | 多线索版 tmaze | 从真假混杂的候选里**挑得准** | 🔧 脚本已写<br>`tmaze_multiclue_test.py` | — |
| **③** | 信用精度 | **压完之后信用还分得对** ← 核心证据 | ⬜ 待建 | ★ |
| **②** | 信号可信度 | 探针问出来的"能不能成"靠不靠谱 | ⬜ 待建 | M1（真实环境） |
| **⑤** | CA 侧消融 | §3 那四条改法各自值不值 | ⬜ 待建 | ③、★ |
| **④** | `value_tree_test` 升级 | 三档 + 固定 token 上限 + 真建树 | ⬜ 待做 | ③（要用它的评估框架验） |

**推荐动手顺序**：⑥（最便宜，且可能推翻 §2.4）→ ★ → ③ → ②（等 M1）→ ⑤ → ④

**⚠️ 已完成的只有 ①，而它只兑现了动机一**（省 token）。**动机二（压缩提纯信号）到现在一条直接证据都没有** —— 那正是 ③ 要补的洞。
...
### ③ `credit_rank_test.py` —— 核心证据 ⬜【待建，最大的洞】

**先说清"信用分配"是在干什么**：一条轨迹跑完只有一个总分（成功=1 / 失败=0）。信用分配就是**把这一个总分拆开、摊到每一步上**，给每步一个数 $A_t$ 表示"这步立了多少功"。$A_t$ 大 = 这步是功臣，$A_t$ 负 = 这步拖了后腿。

**这个实验回答**：

> **压缩之后，功劳还分得对吗？**

**为什么说它是"最大的洞"**：现有两个实验测的都不是信用分配本身 ——

```
   ①测的  →  能不能认出同一处境（上游，sep-AUC）
             ↓
        【信用分配 ← 从没量过】
             ↓
   ①测的  →  任务成功率（下游）
...
上下游都测了，**中间那一环一次都没量过**。而"为信用而压"正是本方法区别于所有现有压缩方法的地方 —— **不测它，这个主张就只停留在嘴上**。

**怎么做（三步）**：

1. **造一个"标准答案已知"的任务** —— 我们事先就知道哪一步是关键步：

   | 任务 | 关键步是哪一步 |
   |---|---|
   | Passive T-Maze | 第 0 步（读线索那步） |
   | Key-to-Door | 拿钥匙那步 |
   | 多线索版 | 那几条真线索所在的步 |

2. 用**四种记忆**分别跑一遍信用分配：full / naive / trunc-$k$ / **vtree**；
3. 看**关键步拿到的功劳**有多少。
...
| 指标 | 定义 | 为什么需要 |
|---|---|---|
| 关键步排名 | 它的 $A_t$ 在所有步里排第几 | 直观。但 full 和 vtree **都排第 1 时就饱和了**，分不出高下 |
| **信用集中度** | $A_{\text{key}}\big/\sum_t \lvert A_t\rvert$ | 不会饱和。无关步分走的功劳越少，这个值越高 |
...
| 结果 | 证明了什么 |
|---|---|
| `vtree ≈ full` | 只证明**动机一**：压了不亏，token 省下来了 |
| **`vtree > full`**（集中度 / margin 显著更高） | **才证明动机二**：压缩**主动提纯**了信用信号 |

§1.2 里 `value_tree_test` 已经在**上游指标**上看到反超（sep-AUC 0.969 > 0.939）；这个实验是要在**信用分配本身**上把它坐实。

---

### ④ `value_tree_test.py` 升级 ⬜【待做，工作量最大】

**这不是一个新实验，是把旧原型改成方案定版的样子。**

那个脚本是最早写的原型，当时很多设计还没定，所以它和现在的方案对不上：

| | 旧原型现在的样子 | 要改成 | 为什么 |
|---|---|---|---|
| 压多少 | `KEEP_RATIO=0.4`，**留固定比例 40%** | **固定 token 上限** | 固定比例下 token 仍随轨迹变长而涨 → 照样超限 → 动机自毁（§1.2） |
| 几档 | **两档**：要么原文、要么删 | **三档**：原文 / 摘要 / 折叠 | 两档下 token 零头用不掉，且保真度没有"在但不可靠"这个状态（§2.4） |
| 树 | **没有** —— 就是个平的 top-k 筛选 | 连续低跳幅段捏成节点，**真建出树** | 方案叫"记忆树"，实现却是平的筛选器，名实不符（§2.6） |

**它测的指标叫 sep-AUC（可分性）**：把压缩后的记忆编码成向量，看"两步的向量像不像"能不能预测"这两步是不是同一个处境"。

- 高（接近 1）= 压完之后仍能认出同处境 → 下游才凑得成组；
- 0.5 = 全糊成一团，谁跟谁都分不清。
...
|---|---|---|
| raw（全历史，不压） | 0.939 | 参照 |
| naive（笨摘要） | 0.499 | 等于瞎猜——压糊了 |
| **vtree（本方法）** | **0.969** | **比不压还高** |

**vtree > raw 这 0.03 值得写进论文**：全历史里的填充内容（"环顾四周""检查了冰箱"）不只浪费 token，还会**干扰**对处境的判断；压掉之后信号更纯。这和 MemoPilot 报告的"Full History 反而更差"是同一个现象，但在这里被**定量化**了 —— 这是动机二目前最直接的证据。

**为什么排在最后**：改完之后需要用 ③ 的评估框架来验证"新版是不是真的比旧版好"，所以得等 ③ 建好。
...
**为什么需要它**：Passive T-Maze 只有**一个**关键步（第 0 步的线索），其余全是废话 —— 闭着眼挑都对。它只能证"该留的留住了"，**证不了"从真假混杂的候选里挑对了"**。而 ③ 和 ⑤ 都要求"有压对/压错之分"，所以它是那两个实验的**前提**。
...
### ⑥ 标定档位分 $c_t$ —— 让保真度有依据 🔧【脚本已写：`calibrate_ct.py`，待跑】

**问题**：$c_{\text{summ}}=0.6$、$c_{\text{fold}}=0.3$ 是拍脑袋填的（§3.2②）。它们直接决定保真度，而保真度又进信用分配——**一个没根据的常数在影响训练信号**。

**三种定法，选第三种**：

| 定法 | 怎么算 | 问题 |
|---|---|---|
| ① 按长度比 | $c_t=n_{\text{comp}}/n_{\text{full}}$（压缩后 / 原文的 token 数；摘要实测 $9/21\approx0.43$） | 只反映留了多少**字**，不反映留的是不是**要紧**的字 —— 摘要专保主干，长度比会**系统性低估**它 |
| ② 每步实测 | $c_t=1-\lvert V_{\text{full}}-V_{\text{comp}}\rvert$（原文版与压缩版的价值之差） | 最准，但每步多问一次探针，在线不可行 |
| ⭐ **③ 一次标定** | 用 ② 的公式跑一批数据取均值，**得到常数后固定用** | **② 的准确性 + ① 的零在线成本** |

**做法**：取数百条真实观测，每条分别以**原文 / 摘要 / 折叠**三种形式喂探针，统计价值判断的一致性：
...
**为什么测排序不测数值**：方法只用跳幅比大小，从不用 $V$ 的绝对值。而且模型若对什么都答 0.5~0.6，数值差天然很小，会给出虚高的假象。

**很便宜**：不训练、只跑探针，可与实验②合并做。

**它还顺带回答一个我们不知道的问题**：摘要到底保住了多少判断力？

- 若标定出 $c_{\text{summ}}\approx 0.9$ → 摘要几乎不损失判断力，**中间档很划算**；
- 若 $\approx 0.3$ → 摘要和折叠差不多，**§2.4 那套"三档优于两档"的论证要重新算**。

**这个数不知道，§2.4 的成本收益账就算不准。**
